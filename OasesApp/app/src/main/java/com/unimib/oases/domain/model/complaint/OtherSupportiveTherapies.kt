...
object OtherSupportiveTherapies: ComplaintSupportiveTherapies {
    override val therapies = listOf(
        OtherWithFeverOrPain,
        OtherWithHypotensionOrShock,
        OtherWithHypertensiveEmergency,
        OtherWithConvulsions,
        OtherWithHypoglycemia,
    )
}

data object OtherWithHypoglycemia: SupportiveTherapy(
    TherapyText("""
        ...
        - Adults: D50% 1 ml/kg IV, check RBS after 15 min, repeat if low
        - Children: D10% 5 ml/kg IV, check RBS after 15 min, repeat if low
        
        NB: Sublingual sugar may be used as an immediate first aid measure in managing hypoglycaemia if IV access is impossible or delayed. Place one level teaspoonful of sugar moistened with water under the tongue every 10–20 min.
...
data object OtherWithFeverOrPain: SupportiveTherapy(
    TherapyText("""
...
data object OtherWithHypotensionOrShock: SupportiveTherapy(
    TherapyText("""
...
data object OtherWithHypertensiveEmergency: SupportiveTherapy(
    TherapyText("""
        If hypertensive emergency (BP >180/110 mmHg with symptoms and acute life threatening complications, eg. seizures, visual disturbance, severe headache): give hydralazine 10 mg IV over 20 minutes.
        Check blood pressure regularly, repeat dose after 20-30 minutes if necessary.
    """.trimIndent()),
    { symptoms: Set<Symptom> ->
...
data object OtherWithConvulsions: SupportiveTherapy(
    TherapyText("""
        ...
        If high fever is present, also give paracetamol (avoid oral route until fully conscious) and undress the patient to reduce the fever. If hypoglycemia, also give IV glucose.
    """.trimIndent()),
    { symptoms: Set<Symptom> ->
