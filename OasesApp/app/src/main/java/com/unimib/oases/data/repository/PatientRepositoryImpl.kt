...
class PatientRepositoryImpl @Inject constructor(
    ...
    override suspend fun addPatient(patient: Patient): Outcome<String> {
        ...
        return try {
            ...
            firestoreManager.updatePatient(patient.toEntity())
    ...
    override suspend fun addPatientAndCreateVisit(patient: Patient, visit: Visit): Outcome<PatientAndVisitIds> {
        return try {
            ...
            firestoreManager.addPatient(patient.toEntity(), visit.toEntity())
    ...
    override suspend fun deletePatientById(patientId: String): Outcome<Unit> {
        return try {
            roomDataSource.addToCache(patientId)
            roomDataSource.deleteById(patientId)
            firestoreManager.deletePatient(patientId)
            Outcome.Success(Unit)
        } catch (e: Exception) {
            Outcome.Error(e.message ?: "An error occurred")
        }
    ...
    override fun getHistoryPatients(): Flow<Resource<List<Patient>>> = flow {
        emit(Resource.Loading())
        try {
            // Call the suspend function from firestoreManager
            val entities = firestoreManager.getHistoryPatients()
            // Map entities to domain models
            val patients = entities.map { it.toDomain() }

            emit(Resource.Success(patients))
        } catch (e: Exception) {
            Log.e("PatientRepository", "Error fetching history: ${e.message}")
