...
object TriageEvaluationSerializer {
    fun serialize(triageEvaluation: TriageEvaluation): ByteArray {
        ...
        val pregnancyHeavyBleedingBytes = triageEvaluation.pregnancyHeavyBleeding.toString().toByteArray(Charsets.UTF_8)
        val severeAbdominalPainBytes = triageEvaluation.severeAbdominalPain.toString().toByteArray(Charsets.UTF_8)
        val seizuresBytes = triageEvaluation.seizures.toString().toByteArray(Charsets.UTF_8)
        val alteredMentalStatusBytes = triageEvaluation.alteredMentalStatus.toString().toByteArray(Charsets.UTF_8)
        val severeHeadacheBytes = triageEvaluation.severeHeadache.toString().toByteArray(Charsets.UTF_8)
        val visualChangesBytes = triageEvaluation.visualChanges.toString().toByteArray(Charsets.UTF_8)
        val sbpHighDpbHighBytes = triageEvaluation.sbpHighDpbHigh.toString().toByteArray(Charsets.UTF_8)
        val traumaBytes = triageEvaluation.trauma.toString().toByteArray(Charsets.UTF_8)
        val activeLaborBytes = triageEvaluation.activeLabor.toString().toByteArray(Charsets.UTF_8)
    ...
    fun deserialize(bytes: ByteArray): TriageEvaluation {
        ...
        return TriageEvaluation(
            ...
            pregnancyHeavyBleeding = pregnancyHeavyBleeding.toBooleanStrict(),
            severeAbdominalPain = severeAbdominalPain.toBooleanStrict(),
            seizures = seizures.toBooleanStrict(),
            alteredMentalStatus = alteredMentalStatus.toBooleanStrict(),
            severeHeadache = severeHeadache.toBooleanStrict(),
            visualChanges = visualChanges.toBooleanStrict(),
            sbpHighDpbHigh = sbpHighDpbHigh.toBooleanStrict(),
            trauma = trauma.toBooleanStrict(),
            activeLabor = activeLabor.toBooleanStrict(),
